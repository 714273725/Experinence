...
public class IndexScroller {
    ...
    public final static int FOCUS_SECTION_CHANGE = 0x01;

    public void setFocusHandler(Handler focusHandler) {
    ...
    public void draw(Canvas canvas) {
        ...
        if (mSections != null && mSections.length > 0) {
            ...
            for (int i = 0; i < mSections.length; i++) {
                if (i == currentSection) {
                    indexPaint.setColor(Color.parseColor("#FF8D12"));
                } else {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN: // 按下，开始索引
                ...
                if (mState != STATE_HIDDEN && contains(ev.getX(), ev.getY())) {
                    ...
                    currentSection = getSectionByPoint(ev.getY());
                    //setFocus();
