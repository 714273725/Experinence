...
import com.jack.myexperience.utils.FileUtil;
import com.jack.myexperience.utils.UriUtil;
...
public class ExtendedBitmapFactory {
    ...
    public static Uri shrinkFile(Uri uri, Context context) throws IOException {
        ...
        if (!FileUtil.saveFile(file, baos.toByteArray())) {
            return null;
        }
    ...
    public static Uri shrink(Uri uri, Context context) throws IOException {
        Bitmap temp = getBitmap(uri, context);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        temp.compress(Bitmap.CompressFormat.PNG, 100, baos);
        baos.flush();
        baos.close();
    ...
    public static Bitmap getBitmap(Uri uri, Context context) throws IOException {
        ...
        Bitmap bitmap = BitmapFactory.decodeFile(uri.getPath(), newOpts);
        ...
        bitmap = BitmapFactory.decodeFile(uri.getPath(), newOpts);
        return bitmap;
    }
